...
contract TicketNFT is ERC721URIStorage {
    ...
    modifier onlyProtocol() {
        if (msg.sender != stakingVaultAddress && msg.sender != lendingPoolAddress) {
    ...
    function setTokenState(uint256 tokenId, uint8 newState) external onlyProtocol {
    ...
    function getFlightDetails(uint256 tokenId) external view returns (FlightDetails memory) {
    ...
    function _update(address to, uint256 tokenId, address auth) internal override returns (address) {
        ...
        if (currentState == State.COLLATERALIZED && msg.sender == lendingPoolAddress) {
            return super._update(to, tokenId, auth);
        }
    ...
    function changeState_TEMPORARY(uint256 tokenId, State newState) external onlyMinter {
